...
from model_utils import CosineClassifier
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, classifier=None, num_classes=64,
                 dropout=0.0, global_pool=True, ext_layer='fc'):
    ...
    def embed(self, x, param_dict=None):
        ...
        x = self.layer1(x)
        x = self.layer2(x)
        if self.ext_layer == 'conv2': 
            return x.reshape(x.shape[0], x.shape[1], -1)
        x = self.layer3(x)
        if self.ext_layer == 'conv3': 
            return x.reshape(x.shape[0], x.shape[1], -1)
        x = self.layer4(x)
        if self.ext_layer == 'conv4': 
            return x.reshape(x.shape[0], x.shape[1], -1)
